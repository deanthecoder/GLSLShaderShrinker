...
float h31(vec3 p3) {
	Clone(ref p3); p3 = fract(p3 * .1031f);
...
vec3 dy(vec3 p) {
	Clone(ref p); p.y += -1.6f;
	return p;
}

float boxFrame(vec3 p) {
	Clone(ref p); p = abs(p) - vec3(1);
	vec3 q = abs(p + .1f) - .1f,
	     v1 = vec3(q.xz, p.y),
	     v2 = vec3(q.xy, p.z),
	     v3 = vec3(q.yz, p.x);
...
Hit sdf(vec3 p) {
	Clone(ref p); Hit h = new Hit(length(p - vec3(0, .5f, 0)) - 1.0f, 4.3f, p);
	U(ref h, boxFrame(p + vec3(0, 1, 0)) - .02f, 2.3f, p);
	float d = p.y + 2.0f;
	d += d < .1f ? smoothstep(.1f, 0.0f, voronoi(p.xz)) * .01f : 0.0f;
	U(ref h, d, 3.3f, p);
...
float shadow(vec3 p, vec3 ld, vec3 n) {
	Clone(ref n); Clone(ref ld); Clone(ref p);if (dot(ld, n) < -.1f) return 0.0f;
	float s = 1.0f,
	      l = .01f,
	      mxL = length(p - lp);
	for (float i = min(iTime, 0.0f); i < 30.0f; i++) {
		float d = sdf(l * ld + p).d;
		s = min(s, mix(50.0f, 7.0f, .8f) * d / l);
		l += max(.05f, d);
		if (mxL - l < .5f || s < .001f) break;
	}
...
float ao(vec3 p, vec3 n) {
	Clone(ref n); Clone(ref p); vec2 h = vec2(.2f, 1);
	vec2 ao = new();
...
float sss(vec3 p, vec3 ld) {
	Clone(ref ld); Clone(ref p); float h = .1f;
	return smoothstep(0.0f, 1.0f, sdf(h * ld + p).d / h);
}

vec3 lights(vec3 p, vec3 ro, vec3 rd, vec3 n, Hit h, float fogAdjust) {
	Clone(ref h); Clone(ref n); Clone(ref rd); Clone(ref ro); Clone(ref p);if (h.id == 0.0f) return sky(rd);
	float _ao, sha, fre, fogY, fg, fogTex,
	      ss = 0.0f,
	      shine = 1.0f;
	vec3 c = new(),
	     l = new(),
...
float addFade() { return min(1.0f, abs(t)); }

vec3 march(vec3 ro, vec3 rd) {
	Clone(ref rd); Clone(ref ro); t = mod(iTime, 30.0f);
	fade = addFade();
	vec3 glassN = new(),
	     dv = new(),
	     n = new(),
	...
	if (progressive_time > 5) {
		...
		col += .2f * clamp(d / 64.0f, 0.0f, 1.0f) * vec3(2, 1.6f, 1.4f) * smoothstep(.5f, 1.0f, pow(dot(rd, normalize(lp)), 3.0f));
	}
