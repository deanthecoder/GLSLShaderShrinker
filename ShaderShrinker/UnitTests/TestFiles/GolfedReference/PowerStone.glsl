// Processed by 'GLSL Shader Shrinker' (Shrunk by 598 characters)
...
mat2 W(_f a) {
	...
	return mat2(u, -u.y, u.x);
}

vec2 P(v2 p, _f n) {
	_f t = 3.141 / n,
	   a = mod(atan(p.y, p.x) + t, 2. * t) - t;
	return LNG(p) * v2(cos(a), sin(a));
}
...
float m(v3 p) {
	...
	p.y--;
	p.xy *= W(-.5);
	_f d = max(k(p, 1.2, .12 * S(p.x + .5)), abs(p.z) - .06);
	p.xy *= W(-2.725);
	p.x += .1;
	p.y -= .05;
	return min(d, max(k(p, .9, .1 * (1.1 - S(p.x - .2))), abs(p.z) - .04));
}
...
Hit J(v3 p) {
	...
	x = abs(p.x) - .6;
	d = max(max(dot(v2(1.5, .6), v2(LNG(p.yz), -x)), -x), max(LNG(p.yz) - .5, 1.2 - abs(p.x)));
	L(Hit(min(d, m(p)), 3));
	s = LNG(p) - 1.;
	d = max(min(LNG(v2(s, dot(sin(p * 26.), cos(p.zxy * 26.)) * .03)) - .02, abs(s + .08) - .05), D.x);
	p.x = O.x < 0. ? min(0., f.y) : max(0., f.y);
	i = mod(floor(atan(-p.y, p.z) * 3.183 + 10.) + 2., 20.);
	tp = X(b * 4. - 2.) * 20.;
	w = max(abs(LNG(p) - .65) - .05, D.y + .01 + .13 * step(i, tp) * min(tp - i, 1.));
	p.yz = P(p.yz, 20.);
	p.y -= .64;
	d = min(d, max(w, -l(p)));
	p = O;
...
float j(v3 p, v3 n, _f h) { return J(h * n + p).d / h; }
...
float z(v3 v) { return exp(dot(v, v) * -.002); }
...
vec3 I(v3 p, v3 U, _f d, Hit h) {
	...
	g = E;
	return mix((F * v3(.43, .29, .52) + G * _ * v3(2.11, 1.69, 1.48)) * c, v3(.05), C);
}
...
vec4 K(inout vec3 p, v3 U, _f s, _f M) {
	...
	for (i = z0; i < s; i++) {
		...
		d += h.d;
		if (d > M) return vec4(0);
		p += h.d * U;
	}

	_f Q = mix(1., .3, (sin(T) * .5 + .5) * SS(13., 15., T));
	return vec4(pow(g, Q) * v3(.73, .5, .88) + I(p, U, d, h), h.id);
}
